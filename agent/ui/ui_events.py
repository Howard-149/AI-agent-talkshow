"""Publish talkshow/ui data-channel events (roster, queue, hand-raise, captions)."""
...
async def emit_role_active(role: str, *, emotion: str | None = None) -> None:
    ...
    await publish_ui_event("role_active", **fields)
...
async def emit_transcript(
    ...
    *,
    ...
    text = text.strip()
    if not text and not (texts and any(v.strip() for v in texts.values())):
        return
    payload: dict = {
        "role": role,
        "speaker": load_persona_name(role) if role != "human" else "Human guest",
        "text": text or (texts or {}).get("en", ""),
        "final": final,
        "step": step,
    }
    if texts:
        cleaned = {k: v.strip() for k, v in texts.items() if v and v.strip()}
        if cleaned:
            payload["texts"] = cleaned
    await publish_ui_event("transcript", **payload)
