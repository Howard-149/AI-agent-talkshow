"""Post-connect session wiring: control channel, opening, roster, and UI flush."""
...
from agent.ui.control_handler import idle_topic_loop, register_control_channel
from agent.data import TalkShowData
from agent.ui.participant_display import publish_agent_panel_metadata, set_agent_display_name
from agent.ui.room_connect import wait_for_local_participant
from agent.session.session_opening import run_session_opening
from agent.floor import TurnController
from agent.adapters.avatar_video import get_avatar_video_publisher
from agent.ui.ui_events import emit_panel_roster, flush_pending_ui_events
...
async def bootstrap_after_connect(
    ...
    try:
        ...
        # Warm multimodal / audio path before welcome + first human turn.
        try:
            await data.runtime.gemma_client.warm_multimodal()
